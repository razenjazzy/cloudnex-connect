...
export const ENV_PARAMS: EnvParam[] = [
  ...
  { key: 'LINE_SECOND_WEBHOOK', requiredIn: [], note: 'Default false; POST /webhook-alt 404s unless true (same handleWebhook)' },
  { key: 'MONGO_USERS', requiredIn: [], note: 'Default false; Mongo identity SoR when true; fail closed without MONGODB_URI' },
  { key: 'GRAPHQL_LINE_INGEST', requiredIn: [], note: 'Default false; ingestLineEvents then processLineMessageJob' },
  ...
  { key: 'PUBLIC_BASE_URL', requiredIn: ['staging', 'production'], note: 'https://host[/site]; path prefixes Admin/Demo. HMAC /webhook and /verify stay on the host origin' },
  { key: 'PUBLIC_ADMIN_BASE', requiredIn: [], note: 'Admin prefix; exact form /admin (VPS URL /cloudnex-connect/admin)' },
  { key: 'PUBLIC_DEMO_BASE', requiredIn: [], note: 'Demo prefix; exact form /demo (redirects into Admin /testing)' },
