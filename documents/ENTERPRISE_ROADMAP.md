...
| **Overall architecture / design quality** | 10/10 (was 8) | The ERP boundary is a real adapter+registry, fail-closed for unimplemented providers, wired into every commerce/directory/catalog/reporting handler; `CLAUDE.md` documents it. Firestore/Odoo/`src/index.ts`/`templates.ts`/**and now `src/demo/page.ts`** are all barrel-split into domain modules — the 836-line HTTP entrypoint is a ~35-line orchestrator (Track B3), the 975-line Flex-builder file is 6 domain files + a barrel (Track B4a), and the 909-line demo control-panel page is now `styles.ts`/`markup.ts`/`script.ts` + a ~25-line assembler (Track B4b, closing out Track B4 entirely) — all verified via live local smoke tests plus a byte-diff of rendered output against the pre-split page, not just build/test. The B4b split also caught and fixed a real, previously-shipped bug: the demo page's `login-token` button had no `addEventListener` wrapper at all (an orphaned callback body — a syntax error that silently broke the entire inline `<script>` block in the browser, so *no* demo-page button worked, not just login). `salesTier` is a real additive layer on top of the protected chain, never replacing it. Track A3's audit sweep is done too. **Track C2 is now closed**: the standalone `src/ux/command-registry.ts` + `src/services/command-policy.ts` foundation (7 hardcoded commands, never wired into anything, drifted out of sync with `service-catalog.ts`'s real ~43-entry map) was retired rather than built out into a second parallel menu system — `service-catalog.ts` already was the single source of truth CLAUDE.md names for both NAV menus and command gating. The one field worth merging (`requiresOtp`) moved onto `COMMAND_PREFIX_SERVICE_MAP` and now feeds `src/line/handlers/action-otp.ts`'s step-up gate directly (`isOtpGatedCommand`), replacing a second hand-maintained prefix list that had to be kept in sync by hand — closing exactly the class of drift bug Track A3 found with `DAILY REPORT`/`SEGMENT CUSTOMERS`. `approvalRequired`/`action`/`handlerName`/per-command `channels` were dropped, not merged: none had a real consumer even in the original scaffolding (approval is gated directly in `quotation.ts`'s handlers, not by a declarative flag), so carrying them into `service-catalog.ts` would only be inert metadata. No architecture gaps remain. | Nothing scored remaining. |
...
(deliberately deferred, no third language yet), and Track C3 in
`STAGED_IMPLEMENTATION_BACKLOG.md` (a future-ERP-provider contract spike,
intentionally not started — no second provider requirement exists yet).
Track B4 and C2 are both fully closed. Nothing left is a speculative or
