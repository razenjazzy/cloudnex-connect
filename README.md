...
**Release:** v10.0.1 — Odoo LINE OA v7. Admin bilingual overlay, LINE command CRUD, Directory LINE↔Odoo, campaigns EN/TH. Cloud Run `deploy:prod` stays gated.  
...
| Admin | Path |
|---|---|
| Console | `https://amardhaka.io/cloudnex-connect/` |
| Demo | `/cloudnex-connect/testing` |
| Campaigns | `/cloudnex-connect/campaigns` |
| Advanced (disabled) | `/cloudnex-connect/advanced` |
...
npm install
APP_ENV=development npm run dev   # background; https://site.local/cloudnex-connect/admin/
```
...
| `LINE_RICH_MENU_EN/TH/JSON` | Sales OA tray ids from upload script |
| `LINE_CHANNEL_CUSTOMER_RICH_MENU_JSON` | Customer OA tray ids (same layout) |
